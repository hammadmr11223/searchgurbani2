...
function Kalyan() {
    ...
    const getData = async () => {
        await Axios.get('https://backend.searchgurbani.com/api/meta?url=raags/raag-kalyan')
            .then((resData) => {
    ...
    const getAngByAngAudio = async () => {
        setLoader(true)
        await ApiHelper.get(API.getAngByAngAudio + "?path=raags/raag_kalyan")
            .then((resData) => {
                setLoader(false);
    ...
    return (
        <div>            
            <HelmetWrapper
                title={`Raag Kalyan -: searchgurbani.com`}
                description={`Learn about Gurbani Raag Kalyan- searchgurbani.com`}
