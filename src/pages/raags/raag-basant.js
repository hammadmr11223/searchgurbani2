...
function Basant() {
    ...
    const getData = async () => {
        await Axios.get('https://backend.searchgurbani.com/api/meta?url=raags/raag-basant')
            .then((resData) => {
    ...
    const getAngByAngAudio = async () => {
        setLoader(true)
        await ApiHelper.get(API.getAngByAngAudio + "?path=raags/raag_basant" )
            .then((resData) => {
                setLoader(false);
    ...
    return (
        <div>
            <HelmetWrapper
                title={`Raag Basant -: searchgurbani.com`}
                description={`Learn about Gurbani Raag Basant- searchgurbani.com`}
