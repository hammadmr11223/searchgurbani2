...
function Maj() {
    ...
    const getData = async () => {
        await Axios.get('https://backend.searchgurbani.com/api/meta?url=raags/raag-majh')
            .then((resData) => {
    ...
    const getAngByAngAudio = async () => {
        setLoader(true)
        await ApiHelper.get(API.getAngByAngAudio + "?path=raags/raag_majh")
            .then((resData) => {
                setLoader(false);
    ...
    return (
        <div>
            <HelmetWrapper
                title={`Raag Majh -: searchgurbani.com`}
                description={`Learn about Gurbani Raag Majh- searchgurbani.com`}
