...
function Kedara() {
    ...
    const getData = async () => {
        await Axios.get('https://backend.searchgurbani.com/api/meta?url=raags/raag-kedara')
            .then((resData) => {
    ...
    const getAngByAngAudio = async () => {
        setLoader(true)
        await ApiHelper.get(API.getAngByAngAudio + "?path=raags/raag_kedara")
            .then((resData) => {
                setLoader(false);
    ...
    return (
        <div>
            <HelmetWrapper
                title={`Raag Kedara -: searchgurbani.com`}
                description={`Learn about Gurbani Raag Kedara- searchgurbani.com`}
