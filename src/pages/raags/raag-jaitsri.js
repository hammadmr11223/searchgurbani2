...
function Jaitsiri() {
    ...
    const getData = async () => {
        await Axios.get('https://backend.searchgurbani.com/api/meta?url=raags/raag-jaitsri')
            .then((resData) => {
    ...
    const getAngByAngAudio = async () => {
        setLoader(true)
        await ApiHelper.get(API.getAngByAngAudio + "?path=raags/raag_jaitsri")
            .then((resData) => {
                setLoader(false);
    ...
    return (
        <div>
            <HelmetWrapper
                title={`Raag Jaitsri -: searchgurbani.com`}
                description={`Learn about Gurbani Raag Jaitsri- searchgurbani.com`}
