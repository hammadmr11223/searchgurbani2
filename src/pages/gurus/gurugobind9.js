...
import gobind9 from '@/assets/img/content/gobind9.jpg';
import gobind18 from '@/assets/img/content/gobind18.jpg';
import HelmetWrapper from '@/components/CommonHelmet';
import Image from 'next/image';
import Link from 'next/link';

function GuruGobind9() {
    ...
    const getData = async () => {
        await Axios.get('https://backend.searchgurbani.com/api/meta?url=gurus/gurugobind9')
            .then((resData) => {
    ...
    return (
        <div>
            <HelmetWrapper
                title={`Guru Gobind Singh Sahib Ji -9-: searchgurbani.com`}
                description={`Learn about life story of Guru Gobind Singh ji -9- searchgurbani.com`}
            ...
            <section className='section-1'>
                                ...
                                {datas ?
                                    ...
                                    < div className='dates'
                                        dangerouslySetInnerHTML={{
                                            __html: datas.html
                                                ?.replace(/src="assets\/img\/res\/gobind9\.jpg"/g, `src="${gobind9.src}"` )
                                                .replace(/src="assets\/img\/res\/gobind18\.jpg"/g, `src="${gobind18.src}"` )
                                        }}
                                    /> : null}
                            </div>
                        </div>
                ...
                <div className='page-content right-algn-next'><Link href='/gurus/gurugobind8'>Back</Link></div>
                </div>
        </div>
