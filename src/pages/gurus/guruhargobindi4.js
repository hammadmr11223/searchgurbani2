...
import gharg8 from '@/assets/img/content/gharg8.jpg'
import gharg10 from '@/assets/img/content/gharg10.jpg';
import HelmetWrapper from '@/components/CommonHelmet';
import Image from 'next/image';
import Link from 'next/link';

function GuruHarGubind4() {
    ...
    const getData = async () => {
        await Axios.get('https://backend.searchgurbani.com/api/meta?url=gurus/guruhargobindi4')
            .then((resData) => {
    ...
    return (
        <div>
            <HelmetWrapper
                title={`Guru Har Gobind Ji -4-: searchgurbani.com  `}
                description={`Learn about life story of Guru Har Gobind ji -4- searchgurbani.com`}
            ...
            <section className='section-1'>
                                    ...
                                    < div className='dates'
                                        dangerouslySetInnerHTML={{
                                            __html: datas.html
                                                 ...
                                                 ?.replace(/src="..\/assets\/img\/res\/gharg8\.jpg"/g, `src="${gharg8.src}"`)
                                                 .replace(/src="..\/assets\/img\/res\/gharg10\.jpg"/g, `src="${gharg10.src}"`)
                                        }}
                                    /> : null}
                            </div>
                        </div>
                ...
                <div className='page-content right-algn-next'><Link href='/gurus/guruhargobindi3'>Back</Link></div>
                </div>
        </div>
