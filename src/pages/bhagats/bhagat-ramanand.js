...
import ramanand from '@/assets/img/content/ramanand.jpg';
import ramanand1 from '@/assets/img/content/ramanand1.jpg';
...
function Ramanand() {
    ...
    const getData = async () => {
        await Axios.get('https://backend.searchgurbani.com/api/meta?url=bhagats/bhagat-ramanand')
            .then((resData) => {
    ...
    return (
        <div>
            <HelmetWrapper
                title={`Bhagat Ramanand Ji -: searchgurbani.com`}
                description={`Life Story of Bhagat Ramanand Ji  at  searchgurbani.com`}
                                                        ...
                                                        {datas ?
                                                            < div className='static-page'
                                                                dangerouslySetInnerHTML={{
                                                                    __html: datas.html
                                                                        ?.replace(/src="assets\/img\/res\/ramanand\.jpg"/g, `src="${ramanand.src}"` )
                                                                        .replace(/src="assets\/img\/res\/ramanand1\.jpg"/g, `src="${ramanand1.src}"` )
                                                                }}
                                                            /> : null}
