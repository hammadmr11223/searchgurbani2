...
import sFarid from '@/assets/img/content/s_farid.gif';
import Farid from '@/assets/img/content/farid.jpg';
import { Helmet } from 'react-helmet';
import HelmetWrapper from '@/components/CommonHelmet';
import Link from 'next/link';

function SheikhFarid() {
    ...
    const getData = async () => {
        await Axios.get('https://backend.searchgurbani.com/api/meta?url=bhagats/baba-farid')
            .then((resData) => {
    ...
    return (
        <div>
            <HelmetWrapper
                title={`Baba Sheik Farid Ji -: searchgurbani.com`}
                description={`Farid was to Punjabi what Chaucer was to English. He made Punjabi poetry and poetry Punjabi.`}
                                                        ...
                                                        {datas ?
                                                            < div className='static-page'
                                                                dangerouslySetInnerHTML={{
                                                                    __html: datas.html
                                                                        ?.replace(/src="assets\/img\/res\/s_farid\.gif"/g, `src="${sFarid.src}"` )
                                                                        .replace(/src="assets\/img\/res\/farid\.jpg"/g, `src="${Farid.src}"` )
