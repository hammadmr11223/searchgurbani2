...
import namdev from '@/assets/img/content/namdev.jpg';
import namdev1 from '@/assets/img/content/namdev1.jpg';
...
function Beni() {
  ...
  useEffect(() => {
    ...
    const getData = async () => {
        await Axios.get('https://backend.searchgurbani.com/api/meta?url=bhagats/bhagat-beni')
            .then((resData) => {
    ...
    return (
        <div>
            <HelmetWrapper
                title={`Bhagat Beni Ji -: searchgurbani.com`}
                description={`Life Story of Bhagat Beni Ji  at  searchgurbani.com`}
                                                        ...
                                                        {datas ?
                                                            < div className='static-page'
                                                                dangerouslySetInnerHTML={{
                                                                    __html: datas.html
                                                                        ?.replace(/src="assets\/img\/res\/namdev\.jpg"/g, `src="${namdev.src}"` )
                                                                         .replace(/src="assets\/img\/res\/namdev1\.jpg"/g, `src="${namdev1.src}"` )
                                                                }}
                                                            /> : null}
